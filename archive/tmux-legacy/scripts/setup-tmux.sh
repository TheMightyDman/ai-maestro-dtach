...
# Ensure the active client dictates pane size (prevents narrow tmux width when multiple viewers attach)
set -g aggressive-resize on

# Keep copy-mode highlight neutral grey if triggered manually
set -g mode-style "bg=#3a3d41,fg=#ffffff"

# Prevent mouse drags from entering tmux copy-mode (let the web client manage selection)
...
# Sync tmux copy buffer with system clipboard (requires pbcopy/xclip depending on platform)
