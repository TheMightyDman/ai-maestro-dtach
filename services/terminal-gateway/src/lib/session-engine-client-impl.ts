...
export class SessionEngineClient {
  ...
  constructor(config: SessionEngineConfig = {}) {
    this.socketPath = resolveSocketPath(config.socketPath)
  ...
  private async sendRequest<T>(request: IpcRequest): Promise<T> {
    ...
    const message = JSON.stringify({
      ...
      ...(hasParams(request) ? { params: request.params } : {})
    })
...
function hasParams(request: IpcRequest): request is Extract<IpcRequest, { params: unknown }> {
...
function resolveSocketPath(explicit?: string): string {
  ...
  const envSocket = process.env.AIMAESTRO_IPC_SOCKET
  if (envSocket) {
    return envSocket
  }

  const legacyEnvSocket = process.env.AIMAESTRO_ENGINE_SOCKET
  if (legacyEnvSocket) {
    return legacyEnvSocket
  }

  const repoSocket = resolve(
    process.cwd(),
    'services',
    'session-engine',
    'data',
    '.aimaestro',
    'sockets',
    'aimaestro-engine.sock'
  )
  const workspaceSocket = resolve(
    process.cwd(),
    'data',
    '.aimaestro',
    'sockets',
  ...
  const homeSocket = resolve(
    process.env.AIMAESTRO_HOME ?? homedir(),
    '.aimaestro',
    'sockets',
    'aimaestro-engine.sock'
  )
  const localSocket = resolve(
    __dirname,
    '../../../services/session-engine/data/.aimaestro/sockets/aimaestro-engine.sock'
  )
  const candidates = [
    repoSocket,
    workspaceSocket,
  ...
  return '/tmp/aimaestro-engine.sock'
}
